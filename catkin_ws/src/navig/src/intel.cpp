...
void callback(const PointCloud::ConstPtr& msg)
{
  ...
  pass1.setInputCloud (cloud_vox);
  pass1.setFilterFieldName ("z");
  pass1.setFilterLimits (0.0, 6.0);
  //pass1.setFilterLimitsNegative (true);
  pass1.filter (*cloud_pass1);
  std::cout<<"vox done\n";
  ROS_INFO("I heard: [%d] [%d]",cloud_pass1->width, cloud_pass1->height);
  pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ>);
  tree->setInputCloud (cloud_pass1);
  ...
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
  {
    ...
    {
      clusters->points.push_back (cloud_pass1->points[*pit]); //*      
      cloud_cluster->points.push_back (cloud_pass1->points[*pit]); //*
  ...
  for(int i=0; i<j; i++)
      {
        if(xcor[i][2]<2.2 )//&& abs(xcor[i][5]-xcor[i][4]>0.15))
        {
